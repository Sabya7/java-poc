...
public class c2_TransformingSequence extends TransformingSequenceBase {
    ...
    public void transforming_sequence_2() {
        ...
        Flux<String> resultSequence =  numerical_service_2().map(
                x -> {
                    if ( x== 0) {
                        return "=";
                    }
    ...
    @Test
    public void maybe() {
        Mono<String> result = maybe_service().switchIfEmpty(Mono.just("no results"));
    ...
    public void sequence_starts_with_zero() {
        Flux<Integer> result = Mono.just(0).concatWith(numerical_service())
                ;
